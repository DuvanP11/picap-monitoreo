...
def handle_error(e):
    """Garantiza que cualquier error retorne JSON, nunca HTML."""
    from flask import jsonify as _jfy
    code = getattr(e, 'code', 500)
    return _jfy({"ok": False, "error": str(e), "code": code}), code
...
def get_client():
    return clickhouse_connect.get_client(
        host="clickhouse.picap.io",
        port=8443,
        username="dperilla",
        password="ByXCowcr7H4ooRamXIcnnQ",
...
def bloqueos():
    ...
    try:
        ...
        return jsonify(limpiar_nan({
            "desde": desde, "hasta": hasta,
            "alertas":     alertas[:200],
            "bloqueados":  bloqueados[:200],
            "reactivados": reactivados[:200],
            "resumen": {
                "total":        total,
                "alerta":       n_alerta,
                "ok":           n_ok,
...
def reconocimiento():
    ...
    try:
        ...
        # Resumen — query simplificada para evitar errores de tipo
        Q_simple = """
            SELECT
                count()                                  AS total_alertas,
                countIf(tipo_alerta='RF + IMEI')         AS n_rf_imei,
                countIf(tipo_alerta='RF')                AS n_rf,
                countIf(tipo_alerta='IMEI')              AS n_imei,
                countIf(nivel='FOTO_DUPLICADA')          AS n_duplicada,
                countIf(nivel='ALERTA')                  AS n_alerta,
                countIf(nivel='REVISAR')                 AS n_revisar,
                countIf(nivel='POSIBLE')                 AS n_posible,
...
# Clasificación: ESTAFA | POSIBLE_ESTAFA | OK
# ══════════════════════════════════════════════════════════════
...
# ── Palabras clave POSIBLE ESTAFA ─────────────────────────────
KW_POSIBLE = [
    'whatsapp','wp','neki','daviplata','dai','nequi','bancolombia',
    'davivienda','bbva','scotiabank','itau','efecty','baloto','gana',
    'su red','movilred','puntored','celular','numero','número',
    'telegrama','instagram','face','facebook','twitter','tiktok',
    'plata','envio','envío','transfer','giro','deposito','depósito',
    'cuenta','banco de bogotá','occidente','agrario','popular',
    'colpatria','helm','pichincha','falabella','rappi','paypal',
    'mercadopago',
]
...
def _detectar_palabras(texto):
    ...
    hits_posible = [kw for kw in KW_POSIBLE if kw in t]
    if hits_posible:
        return 'POSIBLE_ESTAFA', hits_posible
    return 'OK', []
...
_Q_ESTAFA_BASE = """
WITH
-- CTE 1: bookings del período con cancelación por denuncia/seguridad
--        deduplicados con ROW_NUMBER para evitar duplicados
bk_raw AS (
    SELECT
        b._id,
        b.driver_id,
...
-- CTE 2: solo bookings únicos con evento 26 confirmado
bk AS (
    SELECT * FROM bk_raw
    WHERE rn = 1 AND estado_cancelacion = 26
),
...
def estafa():
    ...
    try:
        ...
        total     = len(rows)
        n_estafa  = sum(1 for row in rows if row['clasificacion'] == 'ESTAFA')
        n_posible = sum(1 for row in rows if row['clasificacion'] == 'POSIBLE_ESTAFA')
        n_ok      = sum(1 for row in rows if row['clasificacion'] == 'OK')

        # Tendencia diaria
        trend_map = {}
        for row in rows:
            fecha = (row.get('fecha_servicio') or '')[:10]
            if not fecha or fecha == '—': continue
            if fecha not in trend_map:
                trend_map[fecha] = {'estafa':0,'posible':0,'ok':0}
            cls = row['clasificacion']
            if   cls == 'ESTAFA':         trend_map[fecha]['estafa']  += 1
            elif cls == 'POSIBLE_ESTAFA': trend_map[fecha]['posible'] += 1
            else:                         trend_map[fecha]['ok']      += 1
        trend = [{'fecha':k,'estafa':v['estafa'],'posible':v['posible'],'ok':v['ok']}
        ...
        return jsonify(limpiar_nan({
            ...
            'resumen': {
                ...
                'posible':     n_posible,
                'ok':          n_ok,
                'pct_estafa':  round(n_estafa/total*100,1) if total else 0,
                'pct_posible': round(n_posible/total*100,1) if total else 0,
            },
            'trend':   trend,
            'top_kw':  top_kw,
            'alertas': rows,
...
def exportar_estafa():
    ...
    try:
        filtro_pais = _estafa_filtro_pais(pais)
        sql = _Q_ESTAFA_BASE.format(desde=desde, hasta=hasta, filtro_pais=filtro_pais)
        ch   = get_client()
        r    = ch.query(sql)
        rows_raw = [dict(zip(r.column_names, row)) for row in r.result_rows]
        rows = [_procesar_fila_estafa(row) for row in rows_raw]
        ...
        n_p = sum(1 for r in rows if r["clasificacion"]=="POSIBLE_ESTAFA")
        n_o = sum(1 for r in rows if r["clasificacion"]=="OK")
        total = len(rows)
...
if __name__ == "__main__":
    print("=" * 56)
    print("  Picap Evasión API  →  http://localhost:5050")
...
TOKEN_SECRET = os.environ.get('TOKEN_SECRET', 'picap_monitor_2026_secret_key_x9z')
...
def _ch_init_users():
    ...
    try:
        ...
        if r.result_rows[0][0] == 0:
            # _dt.utcnow() en lugar de None — ClickHouse no acepta NULL en DateTime
            ch.insert("picapmongoprod.dashboard_users",
                [["admin",
                  hashlib.sha256("picap2026".encode()).hexdigest(),
                  "Administrador", "admin@picap.com", "admin", _dt.utcnow(), 1]],
                column_names=["usuario","password_hash","nombre","email","rol","creado_en","activo"])
...
_ADMIN_EMERGENCY = {
    "admin": {
        "password_hash": hashlib.sha256("picap2026".encode()).hexdigest(),
        "nombre": "Administrador",
        "rol": "admin",
        "email": "admin@picap.com",
    }
}
...
con_tiempos AS (
    SELECT 
        ...
        -- Validar mismo punto (tolerancia 0.001 grados ≈ 100m)
        IF(
...
def _pibox_debe_excluirse(servicio):
    """
    Determina si un servicio debe excluirse del análisis
    ...
    # TODO: Excluir servicios con reserva
    # Esto requiere revisar booking_events o un campo específico
...
def _pibox_analizar_servicio(servicio, ch):
    ...
    if servicio.get('flag_mismo_punto') == 1:
        alertas.append({
            'booking_id': servicio.get('booking_id', ''),
            'piloto_nombre': servicio.get('piloto_nombre', 'N/A'),
            'piloto_id': servicio.get('driver_id', ''),
            ...
            'tipo_vehiculo': servicio.get('tipo_vehiculo', 'N/A'),
            'tipo_alerta': 'GPS',
            'observacion': 'Mismo punto de inicio y finalización',
            'severidad': 'ALTA',
            'monto': servicio.get('monto_pagado', 0),
            'fecha_servicio': servicio.get('fecha_servicio', ''),
            'pais': servicio.get('pais', ''),
